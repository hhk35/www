...
<?
    ...
    */
	...
	if ($mode=="modify")
	{
		...
		$sql = "select * from job where num=$num";   // get target record
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			if ($del_ok[$i] == "y")
			{
				...
				unlink($delete_path); //업로드된 파일 삭제 명령

				$sql = "update job set $field_org_name = '$org_name_value', $field_real_name = '$org_real_value'  where num=$num";
		...
		$sql = "update job set subject='$subject', content='$content' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql = "insert into job (id, name, nick, subject, content, regist_day, hit, is_html, ";
